...
#define DRIVER_NAME "final_driver"
...
static int final_driver_init(void) {
    int result;
    printk("<1>final_driver: started\n");
    ...
    if (result < 0) {
        printk("<1>final_driver: Failed to register character device\n");
        return result;
    }
...
static void final_driver_exit(void) {
    printk("<1>final_driver: removed\n");
    int i;
    for (i = 0; i < 4; i++) {
        gpio_free(gpioPin[i]);
    }
...
module_init(final_driver_init);
module_exit(final_driver_exit);